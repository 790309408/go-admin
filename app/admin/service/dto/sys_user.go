...
/*
获取最高工资的员工id,员工名称、员工职位、员工薪资、所属部门
*/
func FindMaxSalaryInfo() func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		db.Table("sys_user su").Select("su.user_id,su.username,sp.post_name ,sp.basic_salary ,sd.dept_name").
			Joins("inner join sys_post sp  on su.post_id = sp.post_id").
			Joins("inner join sys_dept sd on su.dept_id = sd.dept_id").
			Order("sp.basic_salary desc").
			Limit(1)
		return db
	}
}
