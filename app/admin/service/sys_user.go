...
获取最高薪水的员工信息
*/
func (e *SysUser) GetMaxSalaryInfo(user *models.UserInfo) error {
	err := e.Orm.Debug().
		Scopes(
			dto.FindMaxSalaryInfo(),
		).
		Find(user).Error
	if err != nil {
...
func (e *SysUser) GetUserList(c *dto.SysUserGetPageReq, list *[]models.SysUser, p *actions.DataPermission) error {
	/*
		Scopes 作用域允许你复用通用的逻辑，这种共享逻辑需要定义为类型func(*gorm.DB) *gorm.DB
	*/
	var err error
	var data models.SysUser
	err = e.Orm.Debug().
		Scopes(
			cDto.MakeCondition(c.GetNeedSearch()),            //查询条件封装
			cDto.Paginate(c.GetPageSize(), c.GetPageIndex()), // 分页条件封装
