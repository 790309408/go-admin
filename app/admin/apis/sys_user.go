...
获取最高工资员工信息
*/
func (e SysUser) GetMaxSalaryInfo(c *gin.Context) {
	s := service.SysUser{}
	err := e.MakeContext(c).
		MakeOrm().
	...
	s.GetMaxSalaryInfo(&user)
	e.OK(user, "查询成功")
}

func (e SysUser) GetUserList(c *gin.Context) {
	s := service.SysUser{}
	req := dto.SysUserGetPageReq{}
	//1. e.MakeContext(c) //设置http上下文或日志对象.并返回e
	//2. e.MakeOrm() // 设置数据库连接.并返回e
	//3. e.Bind(&req, binding.Form) // 绑定数据
	//4. e.MakeService(&s.Service) // 设置orm 连接
	err := e.MakeContext(c).
		MakeOrm().
	...
	userlist := []models.SysUser{}
	s.GetUserList(&req, &userlist, p)
